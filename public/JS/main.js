...
const users = [{ id: 1, nom: "Jean", prenom: "Pierre", age: 25,utilisateur:"analyste" }];
...
function showAllUsers() {
  ...
  users.forEach((user) => {
    const newInputs = {
      ...
      Utilisateur: document.createElement("input")
    };
    ...
    for (const [key, value] of Object.entries(newInputs)) {
      ...
      key === "Utilisateur" && value.setAttribute("value", `${user.utilisateur}`);
...
function editUser(id) {
  const newInputs = {
    ...
    utilisateur: document.getElementById(`UtlisateurOfUser${id}`).value
  };
  users.forEach((user) => {
    if (user.id === parseInt(id)) {
      ...
      user.utilisateur = newInputs.age;
    }
  });
}
